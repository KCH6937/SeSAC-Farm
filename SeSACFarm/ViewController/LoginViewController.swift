...
class LoginViewController: UIViewController {
    ...
    @objc func loginButtonClicked() {
        ...
        if loginViewModel.email.value == "" || loginViewModel.password.value == "" {
            oneOptionAlert(title: "알림", message: "아이디 또는 비밀번호를 입력해주세요", buttonTitle: "확인")
            
        } else {
            loginViewModel.postUserLogin {
                self.loginViewModel.isCorrect = true
            ...
            if self.loginViewModel.isCorrect == false {
                self.oneOptionAlert(title: "알림", message: "아이디 또는 비밀번호가 일치하지 않습니다", buttonTitle: "확인")
            }
        }
    }
